...
var gotoCmd = &cobra.Command{
	Use:   "goto [id]",
	Short: "Switch tmux focus to a nucleus's primary pane",
...
// resolveID returns the nucleus ID from args[0] if provided, or detects it from
// the current tmux pane when called with no arguments.
func resolveID(args []string, reg nucleusSvc, tm tmuxSvc) (string, error) {
	if len(args) == 1 {
		return args[0], nil
	}
...
func executeGoto(id string, reg nucleusSvc, tm tmuxSvc) error {
	r, err := reg.Load()
	if err != nil {
		return err
	}
	...
	if primary == nil {
		return fmt.Errorf("nucleus %q has no neurons", id)
	}
	return tm.SelectPane(primary.TmuxTarget)
}
