...
func newDetailModel(addNeuronFn func(nucleusID, neuronType, repoPath, branch, baseBranch string, createWorktree, createBranch bool) error, gotoNeuronFn func(nucleusID, neuronID string) error) ui.Model {
	nuclei := sampleNuclei()
	svc := ui.Services{
...
func TestNeuronAdd_OpenOnA(t *testing.T) {
	m := newDetailModel(func(nucleusID, neuronType, repoPath, branch, baseBranch string, createWorktree, createBranch bool) error { return nil }, nil)
	m2, _ := pressSpecial(m, tea.KeyEnter)
	m3, _ := press(m2, "a")
...
func TestNeuronAdd_SelectTypeAndSubmit(t *testing.T) {
	...
	m := newDetailModel(
		func(nucleusID, neuronType, repoPath, branch, baseBranch string, createWorktree, createBranch bool) error {
